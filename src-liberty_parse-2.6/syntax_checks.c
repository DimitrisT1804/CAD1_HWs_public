	...
	*/
	...
	if( !si2drObjectIsNull(attr,&err) )
	{
		...
		else
		{
			...
			do
			{
				si2drIterNextComplexValue(vals,&vtype, &intgr, &float64, &string, &bool_var, &expr, &err);
				switch( vtype )
				{
				case SI2DR_STRING:
					strcpy(tech,string);
...
static int count_strings_in_list(si2drAttrIdT attr)
{
	...
	while( 1 )
	{
		si2drIterNextComplexValue(vals,&type,&intgr,&fl,&str,&bool_var,&expr,&err);
	...
	if( str && str[0] )
	{
		...
		if(  !strcmp(strx,"scalar") )
		{
			...
			if( !si2drObjectIsNull(vs,&err) )
			{
				...
				si2drIterNextComplexValue(vals, &type, &intgr, &float64, &string,&bool_var,&expr,&err);
				si2drIterQuit(vals,&err);
...
static void get_index_info(si2drAttrIdT index_x, struct liberty_value_data *vd, int dimno)
{
	...
	if( !si2drObjectIsNull(index_x,&err) )
	{
		...
		if( vtype != SI2DR_STRING && vtype != SI2DR_FLOAT64 && vtype != SI2DR_INT32)
		{
      ...
      {
         vd->index_info[dimno] = (LONG_DOUBLE *)calloc(sizeof(LONG_DOUBLE),1);
         vd->dim_sizes[dimno] = 1;
         vd->index_info[dimno][0] = (vtype == SI2DR_FLOAT64)? float64 : intgr;
         if (vd->index_info[dimno][0] < 0.0) {
				sprintf(EB,"%s:%d, The 'index_%d' attribute contains the negative "
                  "value '%e'.",
					   si2drObjectGetFileName(index_x,&err),
					   (int)si2drObjectGetLineNo(index_x,&err),
...
static void check_attr_correspondence( si2drAttrIdT attr, libsynt_attribute_info *ai )
{
		...
		else
		{
			...
			case SI2DR_BOOLEAN:
				if( expr )
					...
					bool_var = si2drSimpleAttrGetBooleanValue(attr,&err);
	...
	else if( t == SI2DR_COMPLEX )
	{
		...
		else
		{
			...
			argnum = 1;
			while( si2drIterNextComplexValue(vals,&type,&intgr,&float64,&string,&bool_var,&expr,&err), type != SI2DR_UNDEFINED_VALUETYPE )
			{
				if( !arg )
				{
